...
if (isset($_GET['id_hotel'])) {
    $idToUpdate = $_GET['id_hotel'];

    $sqlGetData = "SELECT * FROM hotel WHERE id_hotel = '$idToUpdate'";
    $resultGetData = mysqli_query($conn, $sqlGetData);

    if ($resultGetData) {
        $rowData = mysqli_fetch_assoc($resultGetData);
        $namahotel = $rowData['nama_hotel']; 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $namahotel = mysqli_real_escape_string($conn, $_POST["nama_hotel"]);
    $alamat = mysqli_real_escape_string($conn, $_POST["nama_daerah"]);
    $latitude = mysqli_real_escape_string($conn, $_POST["latitude"]);
    $longitude = mysqli_real_escape_string($conn, $_POST["longitude"]);
    
    $sqlUpdate = "UPDATE hotel SET nama_hotel = '$namahotel', alamat = '$alamat', latitude = '$latitude', longitude = '$longitude' WHERE id_hotel = '$idToUpdate'";
...
<head>
  ...
  <title>Pencarihotel.id | Dashboard</title>
...
<div class="wrapper">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="index3.html" class="brand-link">
      ...
      <span class="brand-text font-weight-light">Pencarihotel.id</span>
    </a>
                ...
                <div class="col-lg-8 mx-auto">
                    <div class="card shadow">
                        <div class="card-header bg-gradient-primary shadow-primary border-0">
                            <h6 class="text-white text-uppercase mb-0">Edit Lokasi hotel</h6>
                        </div>
                        <div class="card-body px-4">
                            <div id="map" style="height: 600px;"></div>
                            <form method="post" action="">
                                <input type="hidden" name="id" value="<?php echo $editData['id_hotel']; ?>">

                                <div class="mb-3">
                                    <label for="nama_hotel" class="form-label">Nama hotel:</label>
                                    <input type="text" name="nama_hotel" id="nama_hotel" class="form-control" value="<?php echo $namahotel; ?>" required>
                                </div>
